...
const BETS = [
  ...
  { id: 'dozen1', label: '1st 12', payout: 2 }, { id: 'dozen2', label: '2nd 12', payout: 2 }, { id: 'dozen3', label: '3rd 12', payout: 2 },
]
let injected = false
...
export default function RouletteGame() {
  ...
  const spin = () => {
    ...
    setTimeout(() => {
      ...
      const pay = win ? bet * (odds + 1) : 0
      const net = win ? bet * odds : -bet
      if (win) setBalance(b => b + pay)
      setResult({ num, win, net, payout: pay, odds: odds + ':1' })
      setMsg(win ? '+' + net : '-' + bet)
      setSpinning(false)
    }, 3000)
  }
  ...
  return (
    <div className="max-w-xl mx-auto animate-fadeIn">
      ...
      <div style={{ background: 'radial-gradient(ellipse at 50% 30%, #1a8a3a 0%, #0f6a28 60%, #0a4a1a 100%)', borderRadius: 14, padding: 12, border: '4px solid #3d2b1f' }}>
        <div className="flex justify-center mb-2"><div style={{ width: 160, height: 160, position: 'relative' }}>
        ...
        <div className="grid grid-cols-12 gap-0.5 mb-2 max-w-sm mx-auto">
          <div style={{ background: '#16a34a', color: '#fff', textAlign: 'center', fontSize: 8, fontWeight: 700, padding: '3px 0', borderRadius: 2 }} onClick={() => setSelected('straight')}>0</div>
          {Array.from({ length: 36 }, (_, i) => i + 1).map(n => <div key={n} style={{ background: REDS.has(n) ? '#dc2626' : '#1e293b', color: '#fff', textAlign: 'center', fontSize: 7, fontWeight: 700, padding: '4px 0', borderRadius: 2, cursor: 'pointer' }}>{n}</div>)}
        </div>
        {selected && !spinning && !result && <div className="flex justify-center mb-2" style={{ animation: 'rChip 0.3s ease-out' }}><div style={{ width: 24, height: 24, borderRadius: '50%', background: '#f8fafc', border: '2px solid #475569', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 7, fontWeight: 700, color: '#1e293b' }}>{bet}</div></div>}
        <div className="flex gap-1 justify-center flex-wrap">
          {BETS.map(o => <div key={o.id} onClick={() => setSelected(o.id)} style={{ background: selected === o.id ? '#f4a81d' : (o.id === 'red' ? '#dc2626' : '#334155'), color: '#fff', padding: '4px 8px', borderRadius: 4, fontSize: 8, fontWeight: 700, cursor: 'pointer', border: '1px solid #475569' }}>{o.label}<span style={{ fontSize: 6, opacity: 0.7, marginLeft: 2 }}>{o.payout}:1</span></div>)}
        </div>

...
export default function RouletteGame() {
  ...
  const STEPS = ['ベット選択', 'スピン', '結果']

  const spin = () => {
    if (spinning || selected === null || bet > balance) return
    setSpinning(true); setBalance(b => b - bet); setStepIdx(1)
    ...
    setTimeout(() => {
      ...
      const win = winMap[selected] || false; const pay = win ? bet * (odds + 1) : 0; const net = win ? bet * odds : -bet
      if (win) setBalance(b => b + pay)
      setRes({ num, win, net, payout: pay, odds: odds + ':1' }); setStepIdx(2); setSpinning(false)
    }, 3000)
  }
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fadeIn">
      ...
      <div className="flex gap-1 mb-1 flex-wrap">{[...BETS.filter(b => ['straight','split','street','corner','red','black','even','odd'].includes(b.id))].map(o => (
        <div key={o.id} onClick={() => setSelected(o.id)} style={{ background: selected === o.id ? '#f4a81d' : (o.id === 'red' ? '#dc2626' : o.id === 'black' ? '#1e293b' : '#334155'), color: '#fff', padding: '4px 10px', borderRadius: 4, fontSize: 10, fontWeight: 700, cursor: 'pointer', border: '1px solid #475569' }}>{o.label}<span style={{ fontSize: 8, opacity: 0.7, marginLeft: 2 }}>{o.payout}:1</span></div>))}
      </div>
      ...
      <div style={{ background: 'radial-gradient(ellipse at 50% 30%, #1a8a3a 0%, #0f6a28 60%, #0a4a1a 100%)', borderRadius: 16, padding: 14, border: '4px solid #3d2b1f' }}>
        <div className="flex justify-center mb-3"><div style={{ width: 200, height: 200, position: 'relative' }}>
        ...
        <div className="grid grid-cols-12 gap-0.5 mb-2 max-w-md mx-auto">
          <div style={{ background: '#16a34a', color: '#fff', textAlign: 'center', fontSize: 10, fontWeight: 700, padding: '5px 0', borderRadius: 2, cursor: 'pointer' }} onClick={() => setSelected('straight')}>0</div>
          {Array.from({ length: 36 }, (_, i) => i + 1).map(n => <div key={n} onClick={() => setSelected('straight')} style={{ background: REDS.has(n) ? '#dc2626' : '#1e293b', color: '#fff', textAlign: 'center', fontSize: 9, fontWeight: 700, padding: '5px 0', borderRadius: 2, cursor: 'pointer' }}>{n}</div>)}
        </div>
        <div className="flex gap-1 justify-center flex-wrap mb-2">{BETS.filter(b => ['red','black','even','odd','low','high','dozen1','dozen2','dozen3'].includes(b.id)).map(o => (
          <div key={o.id} onClick={() => setSelected(o.id)} style={{ background: selected === o.id ? '#f4a81d' : (o.id === 'red' ? '#dc2626' : '#334155'), color: '#fff', padding: '5px 10px', borderRadius: 4, fontSize: 9, fontWeight: 700, cursor: 'pointer', border: '1px solid #475569' }}>{o.label}<span style={{ fontSize: 8, opacity: 0.7, marginLeft: 2 }}>{o.payout}:1</span></div>))}
        </div>
        ...
        {res && !spinning && <div className="p-3 rounded-lg" style={{ background: 'rgba(0,0,0,0.4)' }}>
          {res.win ? <PayoutBox label={res.num + ' (' + (res.num === 0 ? 'Green' : REDS.has(res.num) ? 'Red' : 'Black') + ')'} bet={bet} odds={res.odds} payout={res.payout} net={res.net} /> :
            <div className="text-center text-sm text-red-400"><div className="font-bold text-base">LOSE</div><div>{res.num}</div><div className="text-lg font-bold mt-1"> -${bet.toLocaleString()} </div></div>}
        </div>}
      </div>

      <div className="flex gap-3 mt-3 justify-center">
        <button onClick={spin} disabled={spinning || selected === null || bet > balance} className="px-6 py-3 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm disabled:opacity-30">{spinning ? '回転中...' : '🎡 スピン！'}</button>
        {res && !spinning && <button onClick={() => { setRes(null); setStepIdx(0) }} className="flex items-center gap-2 px-5 py-3 rounded-lg bg-casino-gold/10 border border-casino-gold/30 text-casino-gold font-bold text-sm"><RotateCcw size={16} /> 次へ</button>}
      </div>
      {spinning && <TipBox text="ホイールが回転中！赤/黒/偶数の確率は約48.6%。ストレートアップ(1数字)は2.7%。" />}
    </div>
  )
}

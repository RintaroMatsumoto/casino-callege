...
export default function CrapsGame() {
  useEffect(() => { if (!injected) { injected = true; const s = document.createElement('style'); s.textContent = '@keyframes dShake { 0% { transform: rotate(0deg); } 25% { transform: rotate(12deg) scale(1.05); } 75% { transform: rotate(-12deg) scale(1.05); } 100% { transform: rotate(0deg); } }'; document.head.appendChild(s) } }, [])
  ...
  const roll = () => {
    ...
    setRolling(true); let c = 0; setSi(phase === 'comeout' ? 1 : 2)
  ...
  const start = (t:'pass'|'dontpass') => { setBetType(t); setPhase('comeout'); setBal(b=>b-bet); setRes(null); setSi(1) }
  ...
  return (
    <div className="animate-fadeIn px-2">
      ...
      <div style={{background:'radial-gradient(ellipse at 50% 30%, #1a8a3a 0%, #0f6a28 60%, #0a4a1a 100%)',borderRadius:20,padding:24,border:'6px solid #3d2b1f',position:'relative'}}>
        ...
        {point && <div style={{position:'absolute',top:38,left:'50%',transform:'translateX(-50%)',background:puck?'#f4a81d':'#64748b',color:'#1e293b',padding:'4px 14px',borderRadius:12,fontSize:14,fontWeight:700,zIndex:5,display:'flex',alignItems:'center',gap:4}}><span>{puck?'ON':'OFF'}</span> {point}</div>}
        <div className="flex justify-center gap-6" style={{marginTop:point?65:28}}>
      ...
      <div className="flex gap-3 mt-4 justify-center">
        {phase!=='bet' && phase!=='res' && <button onClick={roll} disabled={rolling} className="px-8 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-base disabled:opacity-30">{rolling?'振っている...':'🎲 ロール！'}</button>}
        {phase==='res' && <button onClick={()=>{setPhase('bet');setPoint(null);setRes(null);setCall(null);setSi(0)}} className="flex items-center gap-2 px-8 py-3 rounded-xl bg-casino-gold/10 border border-casino-gold/30 text-casino-gold font-bold text-base"><RotateCcw size={20}/> もう一度</button>}
      </div>

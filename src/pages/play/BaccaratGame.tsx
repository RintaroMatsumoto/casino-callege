...
export default function BaccaratGame() {
  ...
  const place = (on: 'player' | 'banker' | 'tie') => {
    ...
    setTimeout(() => {
      ...
      const tiePayout = 8
      if (game.result === 'tie') {
        const payout = on === 'tie' ? bet * (tiePayout + 1) : bet
        const net = on === 'tie' ? bet * tiePayout : 0
        setBalance(b => b + payout)
        setLr({ net, label: 'タイ!', payout, odds: on === 'tie' ? '8:1' : '1:1' })
        setMsg(on === 'tie' ? 'タイ! +$' + net : 'タイ...賭け金返却')
      ...
      setHistory(h => [...h.slice(-19), game.result])
      setPhase('result')
    }, 600)
  }
  ...
  return (
    <div className="max-w-xl mx-auto animate-fadeIn">
      ...
      {phase === 'bet' && <div className="flex gap-2 mb-2"><button onClick={() => place('player')} className="flex-1 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-bold text-xs">プレイヤー ${bet}</button><button onClick={() => place('banker')} className="flex-1 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-700 text-white font-bold text-xs">バンカー ${bet}</button><button onClick={() => place('tie')} className="flex-1 py-2 rounded-lg bg-yellow-600 hover:bg-yellow-700 text-white font-bold text-xs">タイ ${bet} (8:1)</button></div>}
      ...
      <div style={{ background: 'radial-gradient(ellipse at 50% 30%, #1a8a3a 0%, #0f6a28 60%, #0a4a1a 100%)', borderRadius: 14, padding: 12, border: '4px solid #3d2b1f', position: 'relative' }}>
        ...
        <div className="mb-3">
          <div className="flex items-center gap-2 mb-1"><span className="text-[9px] font-bold text-cyan-400">BANKER</span>{phase === 'result' && <span className="text-sm font-bold" style={{ color: g.result === 'banker' ? '#22c55e' : '#666' }}>{g.bTotal}</span>}{g.bThird && phase === 'result' && <span className="text-[7px] text-cyan-300">(3枚)</span>}</div>
        ...
        {g.isNatural && phase === 'result' && <div className="flex justify-center my-1"><span className="text-[9px] font-bold text-casino-gold animate-pulse">ナチュラル!</span></div>}
        {phase === 'result' && <div className="flex justify-center my-1"><div style={{ background: g.result === 'banker' ? '#0d948822' : g.result === 'player' ? '#dc262622' : '#6b728022', border: '1px solid ' + (g.result === 'banker' ? '#0d9488' : g.result === 'player' ? '#dc2626' : '#6b7280'), borderRadius: 20, padding: '2px 14px' }}><span className={'text-xs font-bold ' + (g.result === 'banker' ? 'text-cyan-400' : g.result === 'player' ? 'text-red-400' : 'text-gray-400')}>{g.result === 'player' ? 'プレイヤー' : g.result === 'banker' ? 'バンカー' : 'タイ'}の勝ち</span></div></div>}
        <div>
          <div className="flex items-center gap-2 mb-1"><span className="text-[9px] font-bold text-red-400">PLAYER</span>{phase === 'result' && <span className="text-sm font-bold" style={{ color: g.result === 'player' ? '#22c55e' : '#666' }}>{g.pTotal}</span>}{g.pThird && phase === 'result' && <span className="text-[7px] text-red-300">(3枚)</span>}</div>
        ...
        {lr && phase === 'result' && <div className="mt-2 p-2 rounded-lg" style={{ background: 'rgba(0,0,0,0.4)' }}>
          {lr.net >= 0 ? <PayoutBox label={lr.label} bet={bet} odds={lr.odds} payout={lr.payout} net={lr.net} /> :
            <div className="text-center text-xs text-red-400"><div className="font-bold">LOSE</div><div>{lr.label}</div></div>}
        </div>}
      </div>

      {/* Scoreboard */}
      {history.length > 0 && <div className="flex gap-0.5 mt-2 justify-center flex-wrap">
        {history.map((r, i) => <div key={i} style={{ width: 14, height: 14, borderRadius: 3, background: r === 'player' ? '#dc2626' : r === 'banker' ? '#0d9488' : '#6b7280', fontSize: 6, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>{r === 'player' ? 'P' : r === 'banker' ? 'B' : 'T'}</div>)}
      </div>}
